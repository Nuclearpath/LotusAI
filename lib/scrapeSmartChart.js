...
import scrapeWithoutTable from "./scrapeWithoutSelect";
...
export async function scrapeSmartChart(url) {
  try {
    ...
    await Promise.all([
      ...
      page.waitForNavigation({ waitUntil: "networkidle0", waitUntil: "load" }),
    ]);
    ...
    // await page.select("#month_range", "120");
    const selector = await page.$("#month_range")

       await selector?.selectOption("120")
    const mainActivity = await scrapeTable(page);
    // await page.select("#rtid", "2");
    const selector1 = await page.$("#rtid")

       await selector1?.selectOption("2")
    const pricingTrends = await scrapeWithoutTable(page);
    await selector1?.selectOption("3")
    const pricingRatio = await scrapeWithoutTable(page);
    await selector1?.selectOption("4")
    const daysOnMarket = await scrapeWithoutTable(page);
    await selector1?.selectOption("5")
    const sqft = await scrapeWithoutTable(page);
    await selector1?.selectOption("10")
    const supplyAndDemand = await scrapeWithoutTable(page);
